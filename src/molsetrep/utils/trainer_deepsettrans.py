...
class TrainerDeepSetTrans:
    ...
    def train(
        ...
        for epoch in range(self.n_epochs):
            ...
            for metric in self.train_metrics:
                ...
                wandb.log({"Train metrics": round(metric.compute().item(), 3)})
            ...
            for metric in self.valid_metrics:
                ...
                wandb.log({"Valid metrics": round(metric.compute().item(), 3)})
            ...
            if self.explainer:
                ...
                if not self.silent:
                    ...
                    wandb.log({"Train loss": round(self.train_loss.compute(), 3)})
                    wandb.log({"Valid loss": round(self.valid_loss.compute(), 3)})
    ...
    def test(self, test_loader: DataLoader, average_n_epochs: int = 0) -> None:
        ...
        if not self.silent:
            ...
            wandb.log({"Test loss": round(self.test_loss.compute(), 3)})

            for metric in self.test_metrics:
                print(
                    f"Test {type(metric).__name__}:", round(metric.compute().item(), 3)
                )
                wandb.log({f"Test {type(metric).__name__}": round(metric.compute().item(), 3)})
        else:
            result = {
