...
class MultisetTorchTrainer:
    ...
    def valid_step(self, X, X2, y):
        self.model.eval()
        output = self.model(X, X2)
        loss = self.criterion(output, y)
        return output, loss

    def test_step(self, X, X2, y):
        self.model.eval()
        output = self.model(X, X2)
        loss = self.criterion(output, y)
        return output, loss
    ...
    def train(
        ...
        for epoch in range(self.n_epochs):
            for X, X2, y in train_loader:
                ...
                y = y.to(self.device)
                output, loss = self.train_step(X, X2, y)
            ...
            for X, X2, y in valid_loader:
                X = X.to(self.device)
                X2 = X2.to(self.device)
                y = y.to(self.device)
                output, loss = self.valid_step(X, X2, y)
    ...
    def test(self, test_loader: DataLoader, average_n_epochs: int = 0) -> None:
        ...
        for X, X2, y in test_loader:
            X = X.to(self.device)
            X2 = X2.to(self.device)
            y = y.to(self.device)
            output, loss = self.test_step(X, X2, y)
