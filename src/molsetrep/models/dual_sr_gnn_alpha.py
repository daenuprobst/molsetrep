...
class DualSRGNNClassifier(torch.nn.Module):
    def __init__(
        self,
        n_hidden_sets: int,
        n_elements: int,
        ...
        n_classes: int = 2,
        gnn_0: Optional[torch.nn.Module] = None,
        gnn_1: Optional[torch.nn.Module] = None,
        set_layer: str = "setrep",
    ):
        super(DualSRGNNClassifier, self).__init__()
        ...
        if self.gnn_0 is None:
            if self.in_edge_channels > 0:
                ...
                self.gnn_0 = GIN(in_channels, self.n_hidden_channels[0], num_layers)

        if self.gnn_1 is None:
            if self.in_edge_channels > 0:
                self.gnn_1 = GINE(
                    in_channels,
                    n_hidden_channels[0],
                ...
                self.gnn_1 = GIN(in_channels, self.n_hidden_channels[0], num_layers)
    ...
    def forward(self, batch):
        if self.in_edge_channels > 0:
            ...
            out_0 = self.gnn_0(batch.x.float(), batch.edge_index)
            out_1 = self.gnn_1(batch.x.float(), batch.edge_index)

        t_0 = graph_batch_to_set(out_0, batch, self.n_hidden_channels[0])
        t_0 = self.set_rep(t_0)

        t_1 = graph_batch_to_set(out_1, batch, self.n_hidden_channels[0])
        t_1 = self.set_rep(t_1)

        out = self.mlp(torch.cat((t_0, t_1), 1))
