...
def cons_lig_pock_graph_with_spatial_context(
    ligand, pocket, add_fea=2, theta=5, keep_pock=False, pocket_spatial=True
):
    ...
    lig_pock_dist, lig_pock_edge, node_map = edge_ligand_pocket(
        dm, lig_size, theta=theta, keep_pock=keep_pock
    )
    ...
    atoms_raw = (
        np.concatenate([lig_atoms_raw, pock_atoms_raw])
        if len(pock_atoms_raw) > 0
        else lig_atoms_raw
    )
...
def process_dataset(core_path, refined_path, dataset_name, output_path, cutoff):
    ...
    processed_dict = pairwise_atomic_types(
        path, processed_dict, atom_types, atom_types_
    )
    ...
    train_idxs, valid_idxs = random_split(
        len(refined_data), split_ratio=0.9, seed=2020, shuffle=True
    )
